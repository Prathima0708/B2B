...
const Layout = () => {
  ...
  const url = `/users/my-stores?storeType=${
    ...
    location.pathname !== "/b2b/sellerOrder"
      ? "SUPPLIER"
      : "SELLER"
  }`;
  const getStoreList = () => {
    apiService.get("user_service", url, null).then((response) => {
      if (response) {
        if (response.data.length) {
          setStoreList(response.data);
          setStoreSelected(Number(Number(response.data[0].store_id)));
        }
      } else {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 w-full">
        <Header />
        <Main
          storeIdProps={{ storeList, storeSelected }}
