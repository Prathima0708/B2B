  ...
  const getDriverOnSearch = () => {
    ...
    apiServiceAuth
      ...
      .then((response) => {
        ...
        if (response) {
          parseInt(sellerId) === 0
            ? setDriverList(response.data.data.externalDeliveryExecutive)
            : setDriverList(response.data.data.internalDeliveryExecutive);
        } else {
          // notifyError("Something went wrong !!")
        }
      })
  ...
  const addDriver = (flg) => {
    ...
    if (!flg) {
      ...
      payload.vehicle_number =
        edit_driver.vehicleNoPlateUrl && edit_driver.vehicleNoPlateUrl != null
          ? edit_driver.vehicleNoPlateUrl
          : "";
              ...
              <div className="col-span-8 sm:col-span-4">
                <div className="top-0 w-full right-0 py-4 lg:py-8 px-6 grid gap-4 lg:gap-6 xl:gap-6 md:flex xl:flex bg-gray-50 border-t border-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                  <Title
                    ...
                    description={
                      edit_driver === null
                        ? "Add your Driver and necessary information from here"
              ...
              <div className="grid grid-cols-6 gap-3 md:gap-5 xl:gap-6 lg:gap-6 mb-6">
                <div className="col-span-8 sm:col-span-4">
                  <div className="fixed bottom-0 w-full right-0 py-4 lg:py-8 px-6 grid gap-4 lg:gap-6 xl:gap-6 md:flex xl:flex bg-gray-50 border-t border-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                    ...
                    <div className="flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
                      <Button
                        disabled={!(emailValidation && mobilelValidation)}
                        type="submit"
      ...
      <Card className="min-w-0 shadow-xs overflow-hidden bg-white dark:bg-gray-800 mb-5">
        <CardBody>
          <Grid container spacing={3}>
            ...
            <Grid item lg={4} md={4} sm={6} xs={12} className="pt-1"></Grid>
            <Grid item lg={4} md={4} sm={6} xs={12} className="pt-1"></Grid>
          </Grid>
        </CardBody>
      </Card>
                        ...
                        <div className="flex ">
                          <div
                            onClick={() => {
                              ...
                              setMobilelValidation(true);
                              setName(config.name);
                              setEmail_id(config.emailId);
                              setMobile(config.mobile);
                              setDob(config.dob);
                              setVendor_id(config.vendorId);
                              setAddress(config.address.address);
                              setCity(config.address.city);
                              setPincode(config.address.pincode);
