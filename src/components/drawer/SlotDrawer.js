...
const SlotDrawer = ({ slotDetails, onUpdate, selectedDay = dayOptions[0], id }) => {
  const { toggleDrawer, isDrawerOpen } = useContext(SidebarContext);
  const { id: storeId, zoneId } = useParams();

  const slotClosingOption = [0, 15, 30, 45, 60, 75, 90, 105, 120, 135, 150, 165, 180, 195, 210, 225, 240];

  const [slotStartTime, setSlotStartTime] = useState();
  const [slotEndTime, setSlotEndTime] = useState();
  const [slotMargin, setSlotMargin] = useState();
  const [slotStartTimeOption, setSlotStartTimeOption] = useState(startTimeOption);
  ...
  useEffect(() => {
    if (id) {
      ...
      const startTime = startTimeOption.find((time) => time.time === startTimeString);
      const stopTime = stopTimeOption.find((time) => time.time === endTimeString);
      setSlotStartTime(startTime?.id);
      setSlotEndTime(stopTime?.id);
      const margin = slotClosingOption.find((opt) => opt === slotDetails.showSlotMarginMin);
  ...
  return (
    <>
      <div className="w-full relative p-6 border-b border-gray-100 bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
        {slotDetails ? (
          <Title title="Update Slot" description="Updated slot and necessary information here" />
        ) : (
          <Title title="Add Slot" description=" Add slot and necessary information here" />
        )}
      </div>
