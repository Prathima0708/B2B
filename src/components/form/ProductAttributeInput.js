...
const ProductAttributeInput = ({
  ...
  useEffect(() => {
    ...
    if (currentValue) {
      if (dataType === "JSON_LIST")
        setJsonList(JSON.parse(currentValue.enValue));
      if (dataType === "JSON_OBJ")
        setJsonObject(JSON.parse(currentValue.enValue));
      if (dataType !== "JSON_LIST" && dataType !== "JSON_OBJ")
  ...
  const handleChange = (e) => {
    if (isVariant) {
      setProductAttributes((prevData) => {
        const attributeIndex = prevData[index].variantResource.findIndex(
        ...
        if (attributeIndex !== -1) {
          ...
          setValue(e.target.value);
          prevData[index].variantResource.push({
            attributeId,
            enValue: e.target.value,
          });
          return [...prevData];
      ...
      setProductAttributes((prevData) => {
        const attributeIndex = prevData.findIndex(
          (attr) => attr.attributeId === id
        );
        if (attributeIndex !== -1) {
          setValue(e.target.value);
          prevData[attributeIndex] = {
            ...prevData[attributeIndex],
            attributeId,
            enValue: e.target.value,
          };
  ...
  const setProdAttr = (prevData) => {
    setProductAttributes((data) => {
      ...
      if (attributeIndex !== -1) {
        if (dataType === "JSON_LIST")
          data[attributeIndex] = {
            ...
            enValue: JSON.stringify(
              ...prevData.filter((fea) => fea.length > 0)
            ),
          };
          ...
          data[attributeIndex] = {
            ...
            enValue: JSON.stringify(
              ...prevData.filter(
                (spec) => spec.key.length > 0 && spec.value.length > 0
              )
            ),
          };
        data[attributeIndex] = {
        ...
        return [
          ...data,
          { attributeId, enValue: JSON.stringify([...prevData]) },
        ];
    });
  };
            ...
            {i > 0 && i < jsonList.length && (
              <span className="ml-2 text-xl ">
                <Badge
                  ...
                  onClick={() => {
                    setJsonList((prevData) => {
                      setProdAttr([...prevData.filter((index) => index !== i)]);
                      return [...prevData.filter((index) => index !== i)];
                    });
                  }}
                >
                  Delete
      ...
      >
        {enValue ? (
          <div className="flex flex-row">
            ...
            />
            <div
              className="ml-2 self-center text-red-700 text-xl cursor-pointer"
              onClick={() => setUrl("")}
            >
              <FiTrash2 />
            </div>
  ...
  return (
    <div className="grid gap-3 md:gap-5 xl:gap-6 lg:gap-6 mb-6">
      {displayName && (
        <LabelArea label={mandatory ? `${displayName} *` : displayName} />
      )}
      ...
      {dataType !== "JSON_LIST" &&
        ...
        dataType !== "IMAGE_TYPE" && (
          ...
          >
            ...
            />
            {requiredUnits && (
              <span className="ml-2 text-xl mt-2 text-grey-500 dark:text-white">
                {unitValue}
              </span>
            )}
          </div>
